...
public class CommitLog {
    ...
    protected static int calMsgLength(int sysFlag, int bodyLength, int topicLength, int propertiesLength) {
        ...
        final int msgLen = 4 //TOTALSIZE
            ...
            + 0;
        //整体消息长度不定的,是由最上面的TotalSize进行确定的
        return msgLen;
    }
    ...
    private boolean isMappedFileMatchedRecover(final MappedFile mappedFile) {
        ...
        //判断时间戳,如果时间戳是小于文件检查点的时间戳,那么就说明是可靠的
    ...
    public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
        ...
        if (tranType == MessageSysFlag.TRANSACTION_NOT_TYPE
            ...
            if (msg.getDelayTimeLevel() > 0) {
                ...
                // Backup real topic, queueId
                //将原本的消息主题和队列放入属性中,方便从延迟队列取出来的时候,恢复为原本的消息
                MessageAccessor.putProperty(msg, MessageConst.PROPERTY_REAL_TOPIC, msg.getTopic());
                MessageAccessor.putProperty(msg, MessageConst.PROPERTY_REAL_QUEUE_ID, String.valueOf(msg.getQueueId()));
                msg.setPropertiesString(MessageDecoder.messageProperties2String(msg.getProperties()));
        ...
        MappedFile unlockMappedFile = null;
        //获取到当前可以写入的Commitlog文件,从mappedFileQueue中获取,本质上是对应的文件
        MappedFile mappedFile = this.mappedFileQueue.getLastMappedFile();
        putMessageLock.lock(); //spin or ReentrantLock ,depending on store config
        try {
            long beginLockTimestamp = this.defaultMessageStore.getSystemClock().now();
    ...
    public void handleDiskFlush(AppendMessageResult result, PutMessageResult putMessageResult, MessageExt messageExt) {
        // Synchronization flush 判断是同步还是异步刷盘
        if (FlushDiskType.SYNC_FLUSH == this.defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) {
            final GroupCommitService service = (GroupCommitService) this.flushCommitLogService;
            if (messageExt.isWaitStoreMsgOK()) {
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            //那上面的消息体长度 topic长度, 属性长度 存储方式 计算对应的总长度
